...
\tagrefs{prfAX/{fol:axd:qpr:thm:strong-generalization},prfSC/{fol:seq:qpr:thm:strong-generalization},prfND/{fol:ntd:qpr:thm:strong-generalization},prfTab/{fol:tab:qpr:thm:strong-generalization}} apply.
From \iftag{prvEx}
{$\Gamma_n \Proves \lnot !A_n(c_n)$}
{$\Gamma_n \Proves !A_n(c_n)$},
we obtain
...
\begin{explain}
We'll now show that a \emph{complete}, consistent set which is
saturated has the property that \iftag{prvAll}{it contains a
  universally quantified !!{sentence} iff it contains all its
  instances}{}\iftag{defAll,defEx}{}{ and }\iftag{prvAll}{it contains
  an existentially quantified !!{sentence} iff it contains at least
  one instance}{}. We'll use this to show that the !!{structure} we'll
